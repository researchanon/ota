...
writeLineSTM (THandle hdl) str = return ()

writeStrSTM :: THandle -> String -> STM ()
writeStrSTM (THandle hdl) str = runOT $ do
 let lStr =  length str +1
 liftIO $ print ("lStr: " ++ show lStr)
 pos <- liftIO $ hTell hdl
 liftIO $ print ("Pos: " ++ show pos)
 oldStr <- liftIO $ tryReadStr hdl lStr
 liftIO $ hSeek hdl AbsoluteSeek pos
 pos2 <- liftIO $ hTell hdl
 liftIO $ print ("Old: " ++ oldStr)
 liftIO $ print ("new: " ++ str)
 liftIO $ print ("Pos2: " ++ show pos2)
 liftIO $ hPutStrLn hdl str
 let lOld = length oldStr
 liftIO $ print ("lOld:" ++ show lOld)
 if (lOld < lStr)
  then do 
   ...
   onAbort (do 
    ...
    hSeek hdl AbsoluteSeek pos
    pos3 <- liftIO $ hTell hdl
    liftIO $ print ("abort: pos3 " ++ show pos3)
    hPutStr hdl oldStr
    hSeek hdl AbsoluteSeek pos)     
