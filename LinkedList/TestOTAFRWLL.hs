...
import OTAFRWLinkedList
--import CASList
import GenOpsAndSet



fillSet :: OTAFRWLinkedList.ListHandle Int -> [Int] -> IO ()
fillSet set [] = return ()
fillSet set (x:xs) = do
    ...
    (do
      ...
      assert (length listOpsPerThread == nThreads)
        ...
        --  print $ diffUTCTime stop start)
          ...
          putStrLn $ "OTAFairLL\t" ++ show nThreads ++ "\t" ++show time))

thread :: OTAFRWLinkedList.ListHandle Int -> MVar () -> [Op]-> IO ()
