...
import OTATOLinkedList
--import CASList
import GenOpsAndSet



fillSet :: OTATOLinkedList.ListHandle Int -> [Int] -> IO ()
fillSet set [] = return ()
fillSet set (x:xs) = do
    ...
    (do
      ...
      assert (length listOpsPerThread == nThreads)
        ...
        --  print $ diffUTCTime stop start)
          ...
          putStrLn $ "OTATOLL\t" ++ show nThreads ++ "\t" ++show time))

thread :: OTATOLinkedList.ListHandle Int -> MVar () -> [Op]-> IO ()
